...
struct vec3_t { 
  ...
  //- Basic operations:
    ...
    // Addition
  vec3_t operator + ( const vec3_t &r ) {
    vec3_t v = { x + r.x ,
                 y + r.y ,
                 z + r.z };
    return v;
  ...
  void operator /= ( const float s ) {
    ...
    // Dot product
  float dot( const vec3_t r ) {
    return x*r.x + y*r.y + z*r.z;  
  }
  
    // Cross product
  vec3_t cross( const vec3_t r ) {
