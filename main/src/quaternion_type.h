...
struct quat_t {
  ...
  quat_t( float _w, float _x, float _y, float _z ) {
    w = _w;
    v = { _x, _y, _z };
  }
  
    // Scalar and vector components
  quat_t( float _w, vec3_t _v ) {
    w = _w;
    v = _v;
  }
  ...
  void operator /= ( quat_t &r ) {
    ...
    // Axis and angle 
  void setRotation( const bool SMALL_ANG, vec3_t axis, float ang ) {
    ang *= 0.5;
    if( SMALL_ANG ) {
      w = 1 - ang*ang;
      v = ang * axis.norm(); 
  ...
  void setRotation( const bool SMALL_ANG, vec3_t u ) {
    if( SMALL_ANG ) {
      v = 0.5 * u;
      w = 1 - 0.5*v.dot(v);      
    } else {
  ...
  vec3_t rotate( const bool INTO_ANG, vec3_t r ) {
    float cross = 2*w;
    if( INTO_ANG ) {
      cross = -cross;
    }
  ...
  vec3_t axisX( const bool INTO_ANG ) {
    float w_vz = w*v.z;
    float w_vy = w*v.y;
    //
    if( INTO_ANG ) {
  ...
  vec3_t axisY( const bool INTO_ANG ) {
    float w_vz = w*v.z;
    float w_vx = w*v.x;
    //  
    if( INTO_ANG ) {
  ...
  vec3_t axisZ( const bool INTO_ANG ) {
    float w_vy = w*v.y;
    float w_vx = w*v.x;
    //  
    if( INTO_ANG ) {
